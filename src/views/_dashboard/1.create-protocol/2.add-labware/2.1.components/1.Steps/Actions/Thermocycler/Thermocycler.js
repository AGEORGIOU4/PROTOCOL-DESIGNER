...
export const ThermocyclerForm = ({ onClose, onDelete, stepId, stepTitle }) => {
  ...
  const [checkboxStates, setCheckboxStates] = useState({
    themorcyclerState: false,
    themorcyclerProfile: false,
  });
  const [checkToggleStates, setToggleStates] = useState({
    thermoBlock: false,
    lid: false,
    lidPosition: false,
  });
  const [selectTemperature, setTemperatures] = useState({
    thermoBlock: "°C",
    lid: "°C",
  });
  ...
  const handleCheckboxChange = (e) => {
    ...
    if (id === "themorcyclerState") {
      setCheckboxStates({
        themorcyclerState: checked,
        themorcyclerProfile: !checked,
      });
  ...
  const handleTogglesChange = (e) =>
    setToggleStates({ ...checkToggleStates, [e.target.id]: e.target.checked });
  ...
  const handleStepRemoval = (uniqueIdToRemove) => {
    setAddStep((prevSteps) =>
      prevSteps.reduce((acc, item) => {
        ...
        if (item.isCycle) {
          const updatedCycleSteps = item.cycleSteps.filter(
            (cycleStep) => cycleStep.uniqueId !== uniqueIdToRemove,
          );
  ...
  const addStepComponent = () => {
    ...
    setAddStep((prevSteps) => {
      ...
      if (prevSteps.length > 0) isNotFirstStep = true;
          ...
          >
            ...
            {checkboxStates.themorcyclerState && (
              <>
                <CRow className="mt-2">
                  <CCol md={4}>
                    <CFormLabel htmlFor="ThermocyclerBlockToggle1">
                      Thermocycler Block
                    </CFormLabel>
                    <CFormSwitch
                      label={
            ...
            {checkboxStates.themorcyclerProfile && (
              <>
                <CRow className="mt-2">
                  <CCol md={5}>
                    <CFormLabel htmlFor="profileSettings">
                      Profile Settings
                    </CFormLabel>
                  </CCol>
                </CRow>
                ...
                <CRow className="mt-2">
                  <CCol md={4}>
                    <CCol md={8}>
                      <CFormLabel htmlFor="volumeProfileSetting">
                        Volume
                      </CFormLabel>
                      <CFormInput
                        type="number"
                  ...
                  <CCol md={4}>
                    <CCol md={8}>
                      <CFormLabel htmlFor="lidTemperatureProfileSetting">
                        Lid Temperature
                      </CFormLabel>
                      <CFormInput
                        type="number"
                  ...
                  <CCol md={4}>
                    <CFormLabel htmlFor="lidPositionProfileSetting">
                      Lid Position
                    </CFormLabel>
                    <div>Closed</div>
                  </CCol>
                ...
                <CRow className="mt-4">
                  <CCol md={5}>
                    <CFormLabel htmlFor="profileSettingsSteps">
                      Profile Steps
                    </CFormLabel>
                  </CCol>
                </CRow>
                {addStep.length > 0 && (
                  <CRow>
                    ...
                    <CCol md={3}>
                      <CFormLabel htmlFor="temperatureStep">
                        {" "}
                        Temperature
                      </CFormLabel>
                    </CCol>
                ...
                {addStep.map((stepOrCycle, index) => {
                  if (stepOrCycle.isCycle) {
                    ...
                    return (
                      <React.Fragment key={stepOrCycle.uniqueId}>
                        <CRow key={stepOrCycle.uniqueId} className="mt-3">
                          <CCol>
                            <CFormLabel
                              htmlFor={`cycleLabel${stepOrCycle.uniqueId}`}
                        ...
                        {stepOrCycle.cycleSteps.map(
                          (cycleStep, stepIndex, cycleStepsArray) => {
                            const isLastStep =
                              stepIndex === cycleStepsArray.length - 1;
                            const variant = () => {
                              if (stepIndex === 0) return "first";
                              if (stepIndex === cycleStepsArray.length - 1)
                                return "last";
                              return null;
                            };
                              ...
                              <>
                                <CRow key={cycleStep.uniqueId} className="pt-2">
                                  ...
                                  <CCol md={2}>
                                    ...
                                    >
                                      <CFormLabel
                                        htmlFor={`nameStepInput${stepOrCycle.uniqueId}`}
                                        style={{ marginRight: "10px" }}
                                      >
                                        {index + 1}.{stepIndex + 1}
                                      </CFormLabel>
                                  ...
                                  >
                                    <CloseCircle
                                      ...
                                      onClick={() =>
                                        handleStepRemoval(cycleStep.uniqueId)
                                      }
                                      color="#414141"
                                    />
                                    {cycleStepsArray.length > 1 && (
                                      <div
                                        style={{ width: 28, height: "100%" }}
                                      >
                                        <ConnectionStep variant={variant()} />
                                      </div>
                                    )}
                                  </CCol>
                        ...
                        <CRow
                          className={`mt-4 ${isButtonClicked ? "justify-end-force" : "justify-start-force"}`}
                        >
                          <CCol
                            md={2}
                            className={`${isButtonClicked ? "col-center-content" : ""}`}
                          >
                            <CButton
                              className="profile-btn-steps"
                              onClick={() =>
                                addStepToCycle(stepOrCycle.uniqueId)
                              }
                            >
                              <AddCircle
                                size="24"
                    ...
                    return (
                      <CRow
                        key={stepOrCycle.uniqueId}
                        className={stepOrCycle.isNotFirstStep ? "mt-2" : ""}
                      >
                        <CCol md={3}>
                          <div
                        ...
                        >
                          <CloseCircle
                            ...
                            style={{ cursor: "pointer" }}
                            onClick={() =>
                              handleStepRemoval(stepOrCycle.uniqueId)
                            }
                            color="#414141"
                          />
                        </CCol>
                ...
                <CRow className={`mt-4 justify-end-force`}>
                  ...
                  >
                    <CButton
                      className="profile-btn-steps"
                      onClick={addStepComponent}
                    >
                      <AddCircle size="24" style={{ marginTop: "-2px" }} /> Step
                    </CButton>
                    <CButton
                      className="profile-btn-steps"
                      onClick={addCycleComponent}
                    >
                      <AddCircle size="24" style={{ marginTop: "-2px" }} />{" "}
                      Cycle
                    </CButton>
                  </CCol>
                </CRow>
                ...
                <CRow className="mt-2">
                  <CCol md={4}>
                    <CFormLabel htmlFor="ThermocyclerBlockToggle2">
                      Thermocycler Block
                    </CFormLabel>
                    <CFormSwitch
                      label={
                  ...
                  <CCol md={4}>
                    <CFormLabel htmlFor="LidPositionToggle2">
                      Lid Position
                    </CFormLabel>
                    <CFormSwitch
                      label={checkToggleStates.lidPosition ? "Open" : "Closed"}
