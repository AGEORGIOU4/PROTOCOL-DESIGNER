...
import { CCol, CForm, CCardBody, CCard, CFormCheck, CTooltip, CFormInput, CFormSelect, CFormLabel, CMultiSelect, CRow, CButton, CFormSwitch } from '@coreui/react-pro';
...
export const CentrifugeForm = ({ onClose, onDelete, stepId, stepTitle }) => {
    ...
    const handleLabWareChange = (selectedOptions) => {
        ...
        const updatedPairs = labwarePairs.filter(pair =>
            selectedOptions.find(option => option.value === pair.left.id.replace('left-', '')) &&
            selectedOptions.find(option => option.value === pair.right.id.replace('right-', ''))
        );
        ...
        if (selectedOptions.length % 2 === 0) {
            ...
            for (let i = 0; i < selectedOptions.length; i += 2) {
                newPairs.push({
                    left: { id: `left-${selectedOptions[i].value}`, content: selectedOptions[i].text },
                    right: { id: `right-${selectedOptions[i + 1].value}`, content: selectedOptions[i + 1].text },
                });
            }
    ...
    const handleOnDragEnd = (result) => {
        ...
        if (source.droppableId === destination.droppableId) {
            ...
            // Map back to pairs
            newPairs = newPairs.map((pair, index) => ({
                left: source.droppableId === 'leftColumn' ? sourceItems[index] : destinationItems[index],
                right: source.droppableId === 'rightColumn' ? sourceItems[index] : destinationItems[index]
            }));
        }
                ...
                <CCol md={12}>
                    <CForm className="row g-3 needs-validaiton" noValidate validated={validated} onSubmit={handleSubmit}>
                                            ...
                                            {(provided) => (
                                                <div ref={provided.innerRef} {...provided.droppableProps}>
                                                    {labwarePairs.map((pair, index) => (
                                                        <Draggable key={pair.left.id} draggableId={pair.left.id} index={index}>
                                                            {(provided, snapshot) => (
                                                                <div
                        ...
                        <CRow className='mt-'>
                            ...
                            <CCol md={1} style={{ minWidth: '160px', marginRight: '-20px', marginLeft: "30px" }} >
