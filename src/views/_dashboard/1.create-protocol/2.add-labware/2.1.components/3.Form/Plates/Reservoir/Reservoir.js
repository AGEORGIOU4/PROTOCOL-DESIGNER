...
export default function ReservoirSelection({ selectedSlot, selectedLabware, selectedLiquid, liquidVolume, handleClose }) {
    ...
    const row = Array.from({ length: cols }).map((item, col_index) => {
      ...
      return (
        ...
        <CTooltip
          ...
          placement="bottom"
        >
          <CCol key={id} id={id} className="r_selectables" style={{ borderRadius: squared ? '0' : '100%' }} ref={r_ref}></CCol>
        </CTooltip>
