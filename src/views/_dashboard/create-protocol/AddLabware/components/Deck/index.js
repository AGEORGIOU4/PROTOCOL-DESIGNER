                  ...
                  return (
                    <>
                      ...
                      <td key={index} style={{ maxWidth: '20px', textAlign: 'center' }} >
                        <div key={item} style={{ border: '1px solid #f0f0f0', background: isSelected === item.id ? '#01aab1' : '#fff', color: isSelected === item.id ? '#fff' : 'initial' }}><small>{item.name}</small></div>
                        ...
                        <CButton key={item.name} id={item.name} variant="ghost" size="sm" color="danger" style={{ visibility: item.id === 1 || item.id === -1 ? 'hidden' : 'initial' }} onClick={() => handleDelete(item.id, item.name)}><CIcon icon={cilTrash} /></CButton>
                      </td >
                    </>
                  )
                })}
